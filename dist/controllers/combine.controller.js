...
class CombineController {
    ...
    async fetchCompanyBrands(req, res, next) {
        try {
    ...
    async fetchCompanySubcategories(req, res, next) {
        try {
    ...
    async fetchBrandForAll(req, res, next) {
        try {
    ...
    async fetchManufacturerBrands(req, res, next) {
        try {
    ...
    async fetchManufacturerBrandProducts(req, res, next) {
        try {
            const associatedId = req.account?.associatedId;
    ...
    async fetchAccessibiltyAssociatesCollections(req, res, next) {
        try {
    ...
    async roleBasedInfo(req, res, next) {
        try {
    ...
    async fetchCompaniesManufacturers(req, res, next) {
        try {
    ...
    async superAdminLabelStats(req, res, next) {
        try {
    ...
    async companyLabelStats(req, res, next) {
        try {
            // Extract and validate query parameters
            const { date } = req.body;
            if (typeof date !== 'string')
            ...
            const filter = {
                ...
                manufacture_id: { $in: manufacturerIds },
                status: "show"
            };
            ...
            ["brand_id", "product_id", "variant", "user_id"].forEach((query) => {
                if (req.body[query])
                    filter[query] = req.body[query]; // Adds the specific query parameter to the filter criteria.
            });
            // Fetch labels based on the filter criteria and populate the 'user_id' field with user data.
            const labels = await Label
                .find(filter).populate({
    ...
    async manufacturerLabelStat(req, res, next) {
        try {
    ...
    async manufacturerLabelStats(req, res, next) {
        try {
    ...
    async labelVariants(req, res, next) {
        try {
    ...
    async multipleLabelVariants(req, res, next) {
        try {
    ...
    async fetchProductVariants(req, res, next) {
        try {
            const { brand_ids, product_ids } = req.body;
            const filter = {
                brand_id: { $in: brand_ids },
    ...
    async labelCountInRange(req, res, next) {
        try {
            ...
            const filter = {
                manufacture_id: req.account?.associatedId,
                createdAt: {
                    $gte: endDate,
                    $lte: startDate,
                },
            ...
            if (brand_ids.length > 0)
                filter.brand_id = { $in: brand_ids.map(id => new mongoose.Types.ObjectId(id)) };
            if (product_ids.length > 0)
                filter.product_id = { $in: product_ids.map(id => new mongoose.Types.ObjectId(id)) };
    ...
    async fetchAndDownloadCSV(req, res, next) {
        try {
            // Extract and validate query parameters
            const { date, company_ids, manufacturer_ids, brand_ids, product_ids, variants } = req.body;
            console.log(date);
    ...
    //     }
    // }
    async filterAndDownloadLabelCSV(req, res, next) {
        try {
    ...
    //     }
    // }
    async manufacturerLabelFilterAndCSVDownload(req, res, next) {
        try {
    ...
    async warrantiesCSVDownload(req, res, next) {
        try {
    ...
    async reqHelpOnLabelCSVDownload(req, res, next) {
        try {
            const { startDate, endDate, level } = req.body;
            const filter = {
                createdAt: {
            ...
            if (requestHelpOnLabel.length > 0) {
                requestHelpOnLabel.forEach((reqHelp) => {
                    csvStream.write({
                        DS1: reqHelp.DS1,
    ...
    async reportErrorCSVDownload(req, res, next) {
        try {
            const { startDate, endDate, level } = req.body;
            const filter = {
                createdAt: {
...
//# sourceMappingURL=combine.controller.js.map